...
)
{
    ...
    val result: Flow<DataState<ViewState>> = flow {
        ...
        val apiResult = safeApiCall(dispatcher){apiCall.invoke()} //network only

        val value: Any = when (apiResult) {
    ...
    private suspend fun returnCache(markJobComplete: Boolean): DataState<ViewState> {
        ...
        val cacheResult = safeCacheCall(dispatcher){cacheCall.invoke()} //cache only
